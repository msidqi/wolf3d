...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/01/01 11:16:04 by msidqi            #+#    #+#             */
/*   Updated: 2020/01/01 11:16:06 by msidqi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_draw_walls(int x, t_ray_hit wall, SDL_Surface *bmp, t_player *p)
{
	int i;
	int j;
	int counter;
	...
	while (counter < wall.wall_height / 2)
	{
		if (i >= 0)
		{
			put_pixel32(bmp, x, i, get_wall_texture(i - p->height, wall, bmp));
			i--;
		}
		if (j < bmp->h)
		{
			put_pixel32(bmp, x, j, get_wall_texture(j - p->height, wall, bmp));
			j++;
		}
		counter++;
	}
...
static void	ft_game_loop_content(t_sdl_data *sdl_data, t_player *player,
t_map *map, t_text_layer *tl)
{
	while (!sdl_data->quit)
	{
...
int			main(void)
{
	...
	ft_create_player(&player, TILE_WIDTH + 0.5, TILE_HEIGHT + 0.5, DOWN);
	...
	menu_env.tl.text_pos = (SDL_Rect){1240, 690, 100, 100};
