...
/*   Updated: 2019/12/31 08:28:24 by aabouibr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_create_text_layers(t_menu_env *menu_env, t_sdl_data *sdl_data, int flag)
{
	menu_env->tl.text_pos.x = 645;
	menu_env->tl.text_pos.y = 178;
	menu_env->tl.options[0] = TTF_RenderText_Solid(menu_env->tl.font,"START", menu_env->tl.color[flag & 001 ? 1 : 0]);
	menu_env->tl.options[1] = TTF_RenderText_Solid(menu_env->tl.font,"OPTIONS",menu_env->tl.color[flag & 010 ? 1 : 0]);
	menu_env->tl.options[2] = TTF_RenderText_Solid(menu_env->tl.font,"QUIT",menu_env->tl.color[flag & 100 ? 1 : 0]);
	SDL_BlitSurface(menu_env->tl.options[0], NULL, sdl_data->display, &menu_env->tl.text_pos);
	menu_env->tl.text_pos.y += 100;
	menu_env->tl.text_pos.x -= 25;
	SDL_BlitSurface(menu_env->tl.options[1], NULL, sdl_data->display, &menu_env->tl.text_pos);
	menu_env->tl.text_pos.y += 100;
	menu_env->tl.text_pos.x += 55;
	SDL_BlitSurface(menu_env->tl.options[2], NULL, sdl_data->display, &menu_env->tl.text_pos);
...
static void	ft_menu_show(t_sdl_data *sdl_data, int flags[2], t_menu_env	*menu_env, t_map *map)
{
...
void	ft_menu_loop_content(t_sdl_data *sdl_data, int flags[2], t_menu_env	*menu_env, t_map *map)
{
	while (!sdl_data->startgame)
	{
		while (SDL_PollEvent(&sdl_data->event))
		{
			...
			if (sdl_data->event.key.keysym.scancode == SDL_SCANCODE_KP_ENTER || scan_code_up(sdl_data->event) == 40)
